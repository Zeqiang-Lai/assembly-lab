...
int tokenize()
{
    ...
    while (idx < strlen(expr))
    {
        ...
        if (isalpha(ch))
        {
            status = tokenize_func(&idx);
            if (status != SUCCESS)
                return status;
            continue;
        }
        status = tokenize_operator(&idx);
        if (status != SUCCESS)
            return status;
    }
    return SUCCESS;
